...
def axiom_generator_percept_sentence(t, tvec):
    ...
    tils = [ '' if bul else '~' for bul in tvec ]
    percs = [ til + perc + str(t) for til, perc in zip(tils, proposition_bases_perceptual_fluents) ]
    return ' & '.join(percs)
...
def axiom_generator_pits_and_breezes(x, y, xmin, xmax, ymin, ymax):
    ...
    return '{0} <=> ({1})'.format(breeze_str(x, y), ' | '.join(map_adj(x, y, xmin, xmax, ymin, ymax, pit_str)))

def map_adj(x, y, xmin, xmax, ymin, ymax, fn):
    return [ fn(a, b) for a, b in [(x, y), (x, y-1), (x, y+1), (x-1, y), (x+1, y)] if (a >= xmin and a <= xmax and b >= ymin and b <= ymax) ]

def map_all(xmin, xmax, ymin, ymax, fn):
    return [ fn(a, b) for a in range(xmin, xmax + 1) for b in range(ymin, ymax + 1) ]
...
def axiom_generator_wumpus_and_stench(x, y, xmin, xmax, ymin, ymax):
    ...
    return '{0} <=> ({1})'.format(stench_str(x, y), ' | '.join(map_adj(x, y, xmin, xmax, ymin, ymax, wumpus_str)))
...
def axiom_generator_at_least_one_wumpus(xmin, xmax, ymin, ymax):
    ...
    return ' | '.join(map_all(xmin, xmax, ymin, ymax, wumpus_str))
...
def axiom_generator_at_most_one_wumpus(xmin, xmax, ymin, ymax):
    ...
    wumps = map_all(xmin, xmax, ymin, ymax, wumpus_str)
    lis = [ '(~{0} | ~{1})'.format(a, b) for a in wumps for b in wumps if a != b ]
    return ' & '.join(lis)
...
def axiom_generator_only_in_one_location(xi, yi, xmin, xmax, ymin, ymax, t = 0):
    ...
    t := time; default=0
    """
    curloc = state_loc_str(xi, yi, t)
    locs = map_all(xmin, xmax, ymin, ymax, lambda x, y: '~' + state_loc_str(x, y, t))    
    locs.remove('~' + curloc)
    locs.append(curloc)
    return ' & '.join(locs)

proposition_bases_directions = ['north', 'east', 'south', 'west']
...
def axiom_generator_only_one_heading(heading = 'north', t = 0):
    ...
    dirs = [state_heading_north_str(t), state_heading_east_str(t), state_heading_south_str(t), state_heading_west_str(t)]
    lis = [ a if dirs.index(a) == proposition_bases_directions.index(heading) else '~' + a for a in dirs ]
    return ' & '.join(lis)
...
def axiom_generator_have_arrow_and_wumpus_alive(t = 0):
    ...
    return '{0} & {1}'.format(state_have_arrow_str(t), state_wumpus_alive_str(t))
...
def axiom_generator_location_OK(x, y, t):
    ...
    return '{0} <=> (~{1} & ({2} >> ~{3}))'.format(state_OK_str(x, y, t), pit_str(x, y), wumpus_str(x, y), state_wumpus_alive_str(t))
...
def axiom_generator_breeze_percept_and_location_property(x, y, t):
    ...
    # also determines if (atemporal) square is not breezy
...
def axiom_generator_at_location_ssa(t, x, y, xmin, xmax, ymin, ymax):
    ...
    xmin, xmax, ymin, ymax := the bounds of the environment.
    """
    lis = ['({0} & ~{1})'.format(state_loc_str(x,y,t), action_forward_str(t))]
    adj = map_adj(x, y, xmin, xmax, ymin, ymax, lambda x3, y3: (x3, y3))

    if (x - 1, y) in adj:
         lis.append('({0} & ({1} & {2}))'.format(state_loc_str(x - 1, y, t), state_heading_east_str(t), action_forward_str(t)))
    if (x + 1, y) in adj:
         lis.append('({0} & ({1} & {2}))'.format(state_loc_str(x + 1, y, t), state_heading_west_str(t), action_forward_str(t)))
    if (x, y - 1) in adj:
         lis.append('({0} & ({1} & {2}))'.format(state_loc_str(x, y - 1, t), state_heading_north_str(t), action_forward_str(t)))
    if (x, y + 1) in adj:
         lis.append('({0} & ({1} & {2}))'.format(state_loc_str(x, y + 1, t), state_heading_south_str(t), action_forward_str(t)))

    return '{0} <=> ({1})'.format(state_loc_str(x, y, t + 1),' | '.join(lis))
...
def axiom_generator_have_arrow_ssa(t):
    ...
    return '{0} <=> ({1} & ~{2})'.format(state_have_arrow_str(t + 1), state_have_arrow_str(t), action_shoot_str(t))
...
def axiom_generator_wumpus_alive_ssa(t):
    ...
    return '{0} <=> ({1} & ~{2})'.format(state_wumpus_alive_str(t + 1), state_wumpus_alive_str(t), percept_scream_str(t + 1))

#----------------------------------
def samedir(t):
    return ' & '.join(['~' + action_turn_right_str(t), '~' + action_turn_left_str(t)])
...
def axiom_generator_heading_north_ssa(t):
    ...
    forward = '({0} & ({1}))'.format(state_heading_north_str(t), samedir(t))
    return '{0} <=> ({1} | {2} | {3})'.format(state_heading_north_str(t + 1), forward, left, right)
...
def axiom_generator_heading_east_ssa(t):
    ...
    forward = '({0} & ({1}))'.format(state_heading_east_str(t), samedir(t))
    return '{0} <=> ({1} | {2} | {3})'.format(state_heading_east_str(t + 1), forward, left, right)
...
def axiom_generator_heading_south_ssa(t):
    ...
    forward = '({0} & ({1}))'.format(state_heading_south_str(t), samedir(t))
    return '{0} <=> ({1} | {2} | {3})'.format(state_heading_south_str(t + 1), forward, left, right)
...
def axiom_generator_heading_west_ssa(t):
    ...
    forward = '({0} & ({1}))'.format(state_heading_west_str(t), samedir(t))
    return '{0} <=> ({1} | {2} | {3})'.format(state_heading_west_str(t + 1), forward, left, right)
...
def axiom_generator_heading_only_north(t):
    ...
    return '{0} <=> (~{1} & ~{2} & ~{3})'.format(state_heading_north_str(t), state_heading_south_str(t), state_heading_east_str(t), state_heading_west_str(t))
...
def axiom_generator_heading_only_east(t):
    ...
    return '{0} <=> (~{1} & ~{2} & ~{3})'.format(state_heading_east_str(t), state_heading_south_str(t), state_heading_north_str(t), state_heading_west_str(t))
...
def axiom_generator_heading_only_south(t):
    ...
    t := time
    """
    return '{0} <=> (~{1} & ~{2} & ~{3})'.format(state_heading_south_str(t), state_heading_north_str(t), state_heading_east_str(t), state_heading_west_str(t))
...
def axiom_generator_heading_only_west(t):
    ...
    return '{0} <=> (~{1} & ~{2} & ~{3})'.format(state_heading_west_str(t), state_heading_south_str(t), state_heading_east_str(t), state_heading_north_str(t))
...
def axiom_generator_only_one_action_axioms(t):
    ...
    t := time
    """
    actions = [action_climb_str, action_forward_str, action_grab_str, action_shoot_str, action_turn_left_str, action_turn_right_str, action_wait_str]
    lis = []
    for action in actions:
        temp = []
        for nonaction in actions:
            if action != nonaction:
                temp.append('~' + nonaction(t))
        lis.append('({0} <=> ({1}))'.format(action(t), ' & '.join(temp)))
