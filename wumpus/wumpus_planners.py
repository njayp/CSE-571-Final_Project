...
class PlanRouteProblem(search.Problem):
    ...
    def actions(self, state):
        ...
        if getForwardLoc(state) in self.allowed:
    ...
    def result(self, state, action):
        ...
        if action == 'TurnRight':
            return (state[0], state[1], roundDirection(state[2] - 1))
        if action == 'TurnLeft':
            return (state[0], state[1], roundDirection(state[2] + 1))
        if action == 'Forward':
            nextState = getForwardLoc(state)
            return (nextState[0], nextState[1], state[2])
...
class PlanShotProblem(search.Problem):
    ...
    def h(self, node):
        ...
        """
        lis = [ loc for loc in self.allowed for goal in self.goals if (loc[0] == goal[0]) or (loc[1] == goal[1]) ]
        return min([manhattan_distance_with_heading(node.state, loc) for loc in lis])
